...
get_color() {
    echo "$(xrdb -query | grep $1 | cut -f 2)"
}
...
main()
{
    ...
    white='#d8d8d2'
    gray='#282a36'
    dark_gray=$(get_color "*color8")
    light_purple=$(get_color "*color5")
    dark_purple=$(get_color "*color5")
    cyan=$(get_color "*color4")
    green=$(get_color "*color2")
    orange=$(get_color "*color1")
    red=$(get_color "*color1")
    pink=$(get_color "*color1")
    ...
    for plugin in "${plugins[@]}"; do
        ...
        if [ $plugin = "cwd" ]; then
            ...
            IFS=' ' read -r -a colors  <<< $(get_tmux_option "@dracula-git-colors" "cyan dark_gray")
            tmux set-option -g status-right-length 250
